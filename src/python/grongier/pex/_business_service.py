...
class _BusinessService(_BusinessHost):
    ...
    1) Polling business service with an adapter - The production framework at regular intervals calls the adapter’s on_task() method, 
    ...
    2) Polling business service that uses the default adapter - In this case, the framework calls the default adapter's on_task method with no data. 
    ...
    def OnProcessInput(self,  message_input):
        """ DECAPRETED : use on_process_input
