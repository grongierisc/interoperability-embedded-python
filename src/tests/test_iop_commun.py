...
class TestMessageClassification:
    def test_is_message_class(self):
        assert _Common._is_message_class(SimpleMessage) == True
        assert _Common._is_message_class(SimpleMessageNotMessage) == False

    def test_is_pickle_message_class(self):
        assert _Common._is_pickel_message_class(PickledMessage) == True
        assert _Common._is_pickel_message_class(SimpleMessageNotMessage) == False

    def test_is_iris_object_instance(self):
        msg = iris.cls('Ens.Request')._New()
        assert _Common._is_iris_object_instance(msg) == True
        assert _Common._is_iris_object_instance(SimpleMessageNotMessage) == False
        
        msg_job = iris.cls('Ens.Job')._New()
        assert _Common._is_iris_object_instance(msg_job) == False
